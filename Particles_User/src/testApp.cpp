...
void testApp::setup(){
    ...
    // int _backgroundHex , int _captureIncrement = 6 , int _mapSampling = 2
    PixelHitTestHub::Instance()->setup( ofGetWidth() , ofGetHeight(), 0x000000 , 6 , 2  ) ;
}
...
void testApp::setupGui()
{
    ...
    gui->addSlider( "SMOOTHING AMOUNT" , 1.0f , 35.0f , smoothingAmount ) ;
    //smoothingAmount
...
//    if ( name == "LEVELS" ) levels = ((ofxUISlider *) e.widget)->getScaledValue() ;
    ...
    if ( name == "SMOOTHING AMOUNT" )
    {
        smoothingAmount = ((ofxUISlider *) e.widget)->getScaledValue() ;
    }
